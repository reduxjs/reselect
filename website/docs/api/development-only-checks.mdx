...
<InternalLinks.WeakMapMemoize /> (the default memoizer since 5.0) keys its cache
...
- Pass the [`maxSize` option](./weakMapMemoize#bounding-cache-size-with-maxsize) to bound the cache (as of 5.3.0). Note that bounding a selector created by <InternalLinks.CreateSelector /> requires `maxSize` in both `memoizeOptions` and `argsMemoizeOptions`.
- Switch that selector to <InternalLinks.LruMemoize />, which holds a fixed number of results with exact LRU eviction.
