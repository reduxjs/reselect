...
### 2. Per selector by passing an `inputStabilityCheck` option directly to <InternalLinks.CreateSelector />:
  ...
  <!-- START: development-only-stability-checks/perSelector.ts -->
...
```ts title="development-only-stability-checks/perSelector.ts"
...
```js title="development-only-stability-checks/perSelector.js"
import { createSelector } from 'reselect'
...
```
  ...
  <!-- END: development-only-stability-checks/perSelector.ts -->
