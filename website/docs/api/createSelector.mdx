...
As of Reselect 5.0.2, you can create a "pre-typed" version of <InternalLinks.CreateSelector /> where the `state` type is predefined. This allows you to set the `state` type once, eliminating the need to specify it with every <InternalLinks.CreateSelector /> call.
...
<!-- START: createSelector/typedSelectorCreator.ts -->
...
```ts title="createSelector/typedSelectorCreator.ts"
...
```js title="createSelector/typedSelectorCreator.js"
import microMemoize from 'micro-memoize'
import { shallowEqual } from 'react-redux'
import { createSelectorCreator, lruMemoize } from 'reselect'
...
```
  ...
  <!-- END: createSelector/typedSelectorCreator.ts -->
