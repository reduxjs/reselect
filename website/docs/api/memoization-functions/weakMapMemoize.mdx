...
<InternalLinks.DefaultMemoize /> has to be explicitly configured to have a cache
...
1. Set the `maxSize` with <InternalLinks.DefaultMemoize />:
...
### Using `weakMapMemoize` with <InternalLinks.CreateSelector />
...
### Using `weakMapMemoize` with <InternalLinks.CreateSelectorCreator />
