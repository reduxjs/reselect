...
- Use <InternalLinks.WeakMapMemoize /> (as of 5.0.0+). If the arguments are ever-changing primitives, pass its [`maxSize` option](/api/weakMapMemoize#bounding-cache-size-with-maxsize) to bound the cache growth (as of 5.3.0+)

Also note that if the shared selector composes an expensive selector that only depends on the state, the varying arguments will fan out that inner selector's cache and make it re-check its inputs once per distinct argument on every state update. See [Keeping extra arguments away from unary input selectors](/api/weakMapMemoize#keeping-extra-arguments-away-from-unary-input-selectors).
