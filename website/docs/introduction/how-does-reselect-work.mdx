...
1. **Initial Run**: On the first call, Reselect runs all the [input selectors](./getting-started.mdx#input-selectors), gathers their results, and passes them to the [result function](./getting-started.mdx#result-function).
...
![normal-memoization-function](../../static/img/normal-memoization-function.png)

_Standard memoization only compares arguments. If they're the same, it returns the cached result._

#### Memoization with Reselect

![reselect-memoization](../../static/img/reselect-memoization.png)
...
Even when the overall `state` changes, Reselect ensures efficient memoization through its unique approach. The [result function](./getting-started.mdx#result-function) doesn't re-run if the relevant part of the `state` (in this case `state.todos`), remains unchanged. This is due to Reselect's [Cascading Memoization](#cascading-memoization). The first layer checks the entire `state`, and the second layer checks the results of the [input selectors](./getting-started.mdx#input-selectors). If the first layer fails (due to a change in the overall `state`) but the second layer succeeds (because `state.todos` is unchanged), Reselect skips recalculating the result function. This dual-check mechanism makes Reselect particularly effective in Redux applications, ensuring computations are only done when truly necessary.
