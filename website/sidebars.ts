import type { SidebarsConfig } from '@docusaurus/plugin-content-docs'

/**
 * Creating a sidebar enables you to:
 - create an ordered group of docs
 - render a sidebar for each doc of that group
 - provide next/previous navigation

 The sidebars can be generated from the filesystem, or explicitly defined here.

 Create as many sidebars as you want.
 */
const sidebars: SidebarsConfig = {
  // By default, Docusaurus generates a sidebar from the docs folder structure
  tutorialSidebar: [
    // { type: 'autogenerated', dirName: '.' }
    {
      type: 'category',
      collapsed: false,
      label: 'Introduction',
      items: [
        'introduction/getting-started',
        'introduction/how-does-reselect-work'
      ]
    },
    {
      type: 'category',
      collapsed: false,
      label: 'API',
      items: [
        'api/createSelector',
        'api/createSelectorCreator',
        'api/createStructuredSelector',
        {
          type: 'category',
          collapsed: false,
          label: 'Memoization Functions',
          items: [
            'api/memoization-functions/lruMemoize',
            'api/memoization-functions/weakMapMemoize',
            'api/memoization-functions/unstable_autotrackMemoize'
          ]
        }
      ]
    },
    {
      type: 'category',
      label: 'Debugging Tools',
      items: [
        'debugging-tools/development-only-stability-checks',
        'debugging-tools/output-selector-fields'
      ]
    },
    'v5-summary',
    {
      type: 'category',
      label: 'Optimizing Reselect',
      items: [
        'optimizing-reselect/best-practices',
        'optimizing-reselect/common-mistakes',
        'optimizing-reselect/handling-empty-array-results'
      ]
    },
    'FAQ',
    'external-references',
    'related-projects'
  ]

  // But you can create a sidebar manually
  /*
  tutorialSidebar: [
    'intro',
    'hello',
    {
      type: 'category',
      label: 'Tutorial',
      items: ['tutorial-basics/create-a-document'],
    },
  ],
   */
}

export default sidebars
