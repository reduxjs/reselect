...
- [FAQ](#faq)
  ...
  - [Почему мой селектор производит вычисления когда входное состояние остаётся прежним?](#q-why-is-my-selector-recomputing-when-the-input-state-stays-the-same)
...
`defaultMemoize` может использоваться `createSelectorCreator` чтобы [кастомизировать `equalityCheck` function](#customize-equalitycheck-for-defaultmemoize).
...
Если вы не используете Redux и вам необходимо работать с изменяемыми данными, вы можете использовать `createSelectorCreator` для чтобы заменить функцию мемоизации по умолчанию и/или использовать другую функцию  проверки равенства. См. [здесь](#use-memoize-function-from-lodash-for-an-unbounded-cache) и [здесь](#customize-equalitycheck-for-defaultmemoize) для примеров.
...
A: Имейте в виду, что селекторы могут получить доступ к React props, поэтому, если ваши аргументы являются (или могут быть доступны как) React props, вы можете использовать эту функциональность. [Подробности см. здесь.](#accessing-react-props-in-selectors)
...
### Q: Функция мемоизации по-умолчанию не подходит, могу ли я использовать другую?

A: Мы всё же считаем, что это отлично работает для многих случаев использования. См [эти примеры](#customize-equalitycheck-for-defaultmemoize).

### Q: Как проверить селектор?
...
Предполагается использовать для сложного селектора, который может 
иметь много независимых тестов, а также если вы не хотите вручную управлять подсчетом вычислений или создать «фиктивный» селектор для каждого теста.
...
Если входные параметры селектора обновляются операцией, которая всегда возвращает новый объект, он может выполнять ненужные повторные вычисления. См. [здесь](#q-why-is-my-selector-recomputing-when-the-input-state-stays-the-same) для обсуждения плюсов и минусов использования глубокой проверки на идентичность, такой как `Immutable.is`, чтобы устранить ненужные повторные вычисления.

### Q: Могу ли я использовать селектор для нескольких экземпляров компонентов?
...
* Создайте фабричную функцию, которая возвращает новый селектор для каждого экземпляра компонента. Существует встроенная поддержка фабричных функций в React Redux v4.3 или выше. См. [здесь](#sharing-selectors-with-props-across-multiple-component-instances) для примера.
...
* Статический экспорт JSON-представления вашего селкторного дерева для последующего анализа
