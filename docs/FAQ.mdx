...
Check that your memoization function is compatible with your state update function (i.e. the reducer if you are using [Redux](/)). For example, a selector created with [`createSelector`](./api/createSelector.mdx) will not work with a state update function that mutates an existing object instead of creating a new one each time. `createSelector` uses an identity check (`===`) to detect that an input has changed, so mutating an existing object will not trigger the selector to recompute because mutating an object does not change its identity. Note that if you are using Redux, mutating the state object is [almost certainly a mistake](/usage/troubleshooting).

## Why is my selector recomputing when the input state stays the same?

To address unexpected recomputations in your selector, first ensure that `inputStabilityCheck` is set to either `'always'` or `'once'`. This setting aids in debugging by monitoring the stability of your inputs. Additionally, utilize [output selector fields](./api/createSelector.mdx#output-selector-fields) such as `recomputations`, `resetRecomputations`, `dependencyRecomputations`, and `resetDependencyRecomputations`. These tools help identify the source of the issue.

Keep an eye on the `dependencyRecomputations` count. If it increases while `recomputations` remains the same, it suggests that your arguments are changing references but your [input selectors](./introduction/getting-started.mdx#input-selectors) are stable which is typically the desired behavior.
...
Yes. Reselect has no dependencies on any other package, so although it was designed to be used with [Redux](/) it can be used independently. It can be used with any plain JS data, such as typical [React](https://react.dev) state values, as long as that data is being updated immutably.

## How do I create a selector that takes an argument?

Each of the [input selectors](./introduction/getting-started.mdx#input-selectors) you provide will be called with all of the selector's arguments. You can add additional input selectors to extract arguments and forward them to the [result function](./introduction/getting-started.mdx#result-function), like this:
...
When creating a selector that accepts arguments in Reselect, it's important to structure your input and [output selectors](./introduction/getting-started.mdx#output-selector) appropriately. Here are key points to consider:
...
Yes. The built-in `lruMemoize` memoizer works great for a lot of use cases, but it can be customized or swapped out for a different memoizer. See [these examples](./api/createSelectorCreator.mdx#customize-equalitycheck-for-lrumemoize).
...
Because Reselect uses a [two-stage "cascading" memoization](./introduction/how-does-reselect-work.mdx#cascading-memoization), a memoized selector holds _two_ separate caches:

- the `argsMemoize` cache for the arguments passed to the output selector, attached to the selector itself.
- the `memoize` cache for the results of the [input selectors](./introduction/getting-started.mdx#input-selectors), attached to `memoizedResultFunc`.
...
- Use [`weakMapMemoize`](./api/weakMapMemoize.mdx) (as of 5.0.0+). If the arguments are ever-changing primitives, pass its [`maxSize` option](./api/weakMapMemoize.mdx#bounding-cache-size-with-maxsize) to bound the cache growth (as of 5.3.0+)

Also note that if the shared selector composes an expensive selector that only depends on the state, the varying arguments will fan out that inner selector's cache and make it re-check its inputs once per distinct argument on every state update. See [Keeping extra arguments away from unary input selectors](./api/weakMapMemoize.mdx#keeping-extra-arguments-away-from-unary-input-selectors).
...
Another thing you can do if you are using [React-Redux](/react-redux/) is create a custom hook factory function:
...
You can create a custom typed version of [`createSelector`](./api/createSelector.mdx) by defining a utility type that extends the original `createSelector` function. Here's an example:
...
This approach currently only supports [input selectors](./introduction/getting-started.mdx#input-selectors) provided as a single array.

:::

## What if I want to use `createSelector` without memoization?

There may be rare cases when you might want to use `createSelector` for its composition syntax, but without any memoization applied. In that case, create an [identity function](https://en.wikipedia.org/wiki/Identity_function) and use it as the memoizers:
