...
| :------------------------------------------------ | :---------------------------------------------------------------------------------------------------------------------------------------- |
| [`inputStabilityCheck`](#inputstabilitycheck)     | [input selectors](../introduction/getting-started.mdx#input-selectors) that return a new reference on every run, which breaks memoization |
...
| [`cacheSizeCheck`](#cachesizecheck)               | [`weakMapMemoize`](./weakMapMemoize.mdx) caches that grow without bound from ever-changing primitive arguments                            |
...
Individual selectors can override the global setting by passing a `devModeChecks` option to [`createSelector`](./createSelector.mdx):
...
`cacheSizeCheck` can only be configured globally. It runs inside [`weakMapMemoize`](./weakMapMemoize.mdx) itself, which never sees `createSelector`'s options, so passing `devModeChecks: { cacheSizeCheck: ... }` to a selector has no effect.

:::

## `inputStabilityCheck`

Due to how [cascading memoization](../introduction/how-does-reselect-work.mdx#cascading-memoization) works in Reselect, it is crucial that your [input selectors](../introduction/getting-started.mdx#input-selectors) do not return a new reference on each run. If an input selector always returns a new reference, like
...
Since this is a common mistake, [`createSelector`](./createSelector.mdx) runs the input selectors twice during the first call to the selector. If the results appear to be different for the same call, it logs a warning with the arguments and the two different sets of extracted input values.

Turning the check up to `always` is useful while debugging a selector that recomputes more than expected:

{/* START: development-only-checks/inputStabilityCheck.ts */}
...
```ts title="development-only-checks/inputStabilityCheck.ts"
...
```js title="development-only-checks/inputStabilityCheck.js"
import { createSelector } from 'reselect'
...
{/* END: development-only-checks/inputStabilityCheck.ts */}

## `identityFunctionCheck`

Reselect relies on a separation of concerns between extraction and transformation logic:

- **Extraction logic** retrieves data from a broader state, like `state => state.todos`. It belongs in [input selectors](../introduction/getting-started.mdx#input-selectors).
- **Transformation logic** manipulates or formats that data, like `todos => todos.map(({ id }) => id)`. It belongs in the [result function](../introduction/getting-started.mdx#result-function).
...
{/* START: development-only-checks/identityFunctionCheck.ts */}
...
```ts title="development-only-checks/identityFunctionCheck.ts"
import { createSelector } from 'reselect'
...
```js title="development-only-checks/identityFunctionCheck.js"
import { createSelector } from 'reselect'
...
{/* END: development-only-checks/identityFunctionCheck.ts */}

## `cacheSizeCheck`

[`weakMapMemoize`](./weakMapMemoize.mdx) (the default memoizer since 5.0) keys its cache
...
This check counts the distinct primitive values cached for each argument position. When a single position exceeds **1,000** distinct values, it logs a warning naming the memoized function. The count is per argument position, so a per-item selector called with a bounded set of ids will not trip it — the warning fires on the _unbounded_ pattern, not on legitimate wide caches.

If you see this warning, the usual fixes are:

- Pass the [`maxSize` option](./weakMapMemoize#bounding-cache-size-with-maxsize) to bound the cache (as of 5.3.0). Note that bounding a selector created by [`createSelector`](./createSelector.mdx) requires `maxSize` in both `memoizeOptions` and `argsMemoizeOptions`.
- Switch that selector to [`lruMemoize`](./lruMemoize.mdx), which holds a fixed number of results with exact LRU eviction.
