...
[`lruMemoize`](./lruMemoize.mdx) has to be explicitly configured to have a cache
size larger than 1, and uses an LRU cache internally.

`weakMapMemoize` creates a tree of [`WeakMap`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/WeakMap)-based cache nodes based on the identity of the arguments it's been called with (in this case, the extracted values from your input selectors). **This allows `weakMapMemoize` to have an effectively infinite cache size**. Cache results will be kept in memory as long as references to the arguments still exist, and then cleared out as the arguments are garbage-collected.

:::info Default since v5.0.0

Since v5.0.0, `weakMapMemoize` is the **default** memoization function used by [`createSelector`](./createSelector.mdx) for both `memoize` and `argsMemoize`. In v4.x the default was `lruMemoize` (previously named `defaultMemoize`) with a cache size of **1**.
...
- Pros:
  ...
  - It has an effectively infinite cache size, but you have no control over
    how long values are kept in cache as it's based on garbage collection and [`WeakMap`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/WeakMap)s.

- Cons:
  - There's currently no way to alter the argument comparisons. They're based on [strict reference equality](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Strict_equality).
...
1. Set the `maxSize` with [`lruMemoize`](./lruMemoize.mdx):
...
2. Create unique selector instances using [`useMemo`](https://react.dev/reference/react/useMemo).
...
3. Use [Re-reselect](https://github.com/toomuchdesign/re-reselect):
...
[`createSelector`](./createSelector.mdx) memoizes at two levels: `argsMemoize` caches by
...
### Keeping extra arguments away from unary input selectors

When a `createSelector` selector is called, every input selector receives _all_ of the call's arguments — including input selectors that only use the first one. If such an input selector is itself a memoized selector, its cache is still keyed by the full argument list, so every distinct value of the extra argument creates a separate cache entry for it.

The results stay correct, and result functions do not rerun. But each of those cache entries is checked independently: when the state updates, a selector called with `N` distinct ids re-runs its input selectors `N` times instead of once. In a list where every item calls `useSelector(state => selectSomeData(state, id))` and the state updates frequently, that per-update cost grows with the size of the list. The extra cache entries also hold `N` copies of the same result. A telltale sign is `dependencyRecomputations()` on the inner selector increasing by the number of list items on every state update, rather than by 1.

If an expensive input selector only depends on the state, wrap it so the extra arguments never reach it:

```ts no-emit
const selectSomeData = createSelector(
  [
    // Not `selectHeavyComputation` directly: this keeps `id` out of its cache key
    (state: RootState) => selectHeavyComputation(state),
    (state: RootState, id: string) => id
  ],
  (heavyResult, id) => heavyResult[id]
)
```
