...
`props` can be passed through to `getVisibleTodos` from `mapStateToProps`:
...
Using `getVisibleTodos` with multiple instances of the `visibleTodoList` container will not correctly memoize:
