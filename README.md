...
We can now pass `props` to `getVisibleTodos` in `mapStateToProps`:
...
But there is a problem using this version of `getVisibleTodos` with the `visibleTodoList` container—it does not correctly memoize:
